...
9. Lógica de gameplay (`doAttack`, `checkPassiveCollisions`, `scoredHoney`) — acá vive la interacción entre las tres variables. `reachAt()` centraliza el alcance del ataque (mayor en la zona dulce de borrachera, 30–60%) para que `doAttack` y el anillo visual de alcance (`player.attackRing`, ver abajo) siempre coincidan. En la zona dulce `scoredHoney` aplica el multiplicador de puntaje; `updateDrunkFX` refleja ese mismo rango con el aura dorada de `player.aura` y `toast()` acepta un tercer parámetro `big` para el feedback reforzado de miel. La abeja de rescate (`o.userData.healer`) solo se puede spawnear con `vidas === 1` (control en `spawnSegment`, cooldown vía `lastHealerSpawnZ`) y devuelve 1 vida si se la mata en `doAttack`; si no, el sting normal de `checkPassiveCollisions` aplica igual. Cada ataque además dispara `player.attackRingT` (un anillo elíptico que se desvanece en ~0.4s, dorado si conectó / gris si fue al aire) y, si falló, un toast "💨 al aire" — agregado tras feedback de no-gamers de que no se entendía el timing del combate.
